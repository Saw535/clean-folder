...
def handle_zip(filename: Path, target_folder: Path) -> None:
    target_folder.mkdir(exist_ok=True, parents=True)
...
def handle_gz(filename: Path, target_folder: Path) -> None:
    target_folder.mkdir(exist_ok=True, parents=True)
    folder_for_file = target_folder / normalize(filename.name.replace(filename.suffix, ''))
    folder_for_file.mkdir(exist_ok=True, parents=True)
    try:
...
def handle_tar(filename: Path, target_folder: Path) -> None:
    target_folder.mkdir(exist_ok=True, parents=True)
    folder_for_file = target_folder / normalize(filename.name.replace(filename.suffix, ''))
    folder_for_file.mkdir(exist_ok=True, parents=True)
    try:
...
def main(folder: Path) -> None:
    ...
    for file in parser.ZIP_ARCHIVES:
        handle_zip(file, folder / 'archives' / 'ZIP')
    for file in parser.GZ_ARCHIVES:
        handle_gz(file, folder / 'archives' / 'GZ')
    for file in parser.TAR_ARCHIVES:
        handle_tar(file, folder / 'archives' / 'TAR')
